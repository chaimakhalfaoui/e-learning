...
const EditEns = () => {
    ...
    // Charger les données existantes de l'enseignant
    useEffect(() => {
        const fetchEnseignant = async () => {
            setLoading(true);
            try {
                const response = await axios.get(`http://isetso-backend-lb-667158618.us-east-1.elb.amazonaws.com:8801/api/api/enseignants/${id}`);
                setInputs(response.data);
                setErr(null);
            } catch (error) {
                console.error("Erreur récupération enseignant :", error);
                if (error.response?.status === 404) {
                    setErr("Enseignant non trouvé");
                } else {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.put(`http://isetso-backend-lb-667158618.us-east-1.elb.amazonaws.com:8801/api/api/enseignants/${id}`, inputs);
    ...
    return (
        <React.Fragment>
            <Helmet>
                ...
                <title>Modifier Enseignant | ISETSO</title>
            </Helmet>
            <OffWrap />
            <Header
                parentMenu='pages'
