...
const EditEtd = () => {
    ...
    useEffect(() => {
        const fetchUser = async () => {
            setLoading(true);
            try {
                const response = await axios.get(`http://isetso-backend-lb-667158618.us-east-1.elb.amazonaws.com:8801/api/api/etudiants/${id}`);
                setInputs(response.data);
                setErr(null);
            } catch (error) {
                console.error("Erreur récupération étudiant :", error);
                if (error.response?.status === 404) {
                    setErr("Étudiant non trouvé");
                } else {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.put(`http://isetso-backend-lb-667158618.us-east-1.elb.amazonaws.com:8801/api/api/etudiants/${id}`, inputs);
    ...
    return (
        <React.Fragment>
            <Helmet>
                ...
                <title>Modifier Étudiant | ISETSO</title>
            </Helmet>
            <OffWrap />
            <Header
                parentMenu='pages'
