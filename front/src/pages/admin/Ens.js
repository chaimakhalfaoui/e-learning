...
const Ens = () => {
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            try {
                ...
                const response = await axios.get(`process.env.REACT_APP_API_URL/auth/checkUserRoleA/${userId}`);
    ...
    const fetchAllTeachers = async () => {
        try {
            const response = await axios.get('process.env.REACT_APP_API_URL/auth/getAllTeachers');
    ...
    const handleDeleteUser = async (userId) => {
        ...
        try {
            await axios.delete(`process.env.REACT_APP_API_URL/auth/deleteUser/${userId}`);
            fetchAllTeachers();
                ...
                <div className="container">
                    ...
                    {teachers.map((teacher, index) => (
                        <div key={index} className="col-lg-4 col-md-6 mb-30">
                            <SingleTeam
                                itemClass="team-item"
                                Image={teacher.image ? `process.env.REACT_APP_API_URL/image/${teacher.image}` : getRandomImage()}
