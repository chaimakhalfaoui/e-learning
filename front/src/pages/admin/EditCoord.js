...
const EditCoord = () => {
    ...
    // Charger les données existantes du coordinateur
    useEffect(() => {
        const fetchCoordinateur = async () => {
            setLoading(true);
            try {
                const response = await axios.get(`http://isetso-backend-lb-667158618.us-east-1.elb.amazonaws.com:8801/api/coordinateurs/${id}`);
                setInputs(response.data);
                setErr(null);
            } catch (error) {
                console.error("Erreur récupération coordinateur :", error);
                if (error.response?.status === 404) {
                    setErr("Coordinateur non trouvé");
                } else {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.put(`http://isetso-backend-lb-667158618.us-east-1.elb.amazonaws.com:8801/api/coordinateurs/${id}`, inputs);
            toast.success('Coordinateur modifié avec succès !', { 
                autoClose: 3000,
                position: "top-right"
            });
            setTimeout(() => {
                navigate("/admin/coordinateur");
            }, 2000);
        } catch (err) {
    ...
    if (loading) {
        return (
            <React.Fragment>
                ...
                />
                <SiteBreadcrumb
                    pageTitle="Modifier Coordinateur"
                    pageName="Modifier Coordinateur"
    ...
    return (
        <React.Fragment>
            <Helmet>
                ...
                <title>Modifier Coordinateur | ISETSO</title>
            </Helmet>
            <OffWrap />
            <Header
                parentMenu='pages'
                    ...
                    <div style={formContainerStyle}>
                        <div className="sec-title text-center mb-30">
                            ...
                            <p className="desc" style={{ color: '#666' }}>
                                Modifiez les informations du coordinateur
                            </p>
                        </div>
