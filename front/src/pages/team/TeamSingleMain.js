...
const TeamSingleMain = () => {
    ...
    const getDataAdmin = useCallback(async () => {
        ...
        try {
            const userId = await idUser();
            if (!userId) return;
            const response = await axios.get(`http://isetso-backend-lb-667158618.us-east-1.elb.amazonaws.com:8801/api/auth/getAdminById/${userId}`);
            if (mountedRef.current) {
    ...
    const handleSaveClick = async (e) => {
        ...
        try {
            await axios.put(`http://isetso-backend-lb-667158618.us-east-1.elb.amazonaws.com:8801/api/auth/updateprofil/${userData.id}`, formData, {
                headers: {
    ...
    const handleSaveClickAdmin = async (e) => {
        ...
        try {
            await axios.put(`http://isetso-backend-lb-667158618.us-east-1.elb.amazonaws.com:8801/api/auth/updateAdminById/${adminData.id}`, updatedData);
            setIsEditing(false);
            await getDataAdmin();
        } catch (error) {
            console.error('Erreur:', error);
                                    ...
                                    <label style={imageLabelStyle}>
                                        {(previewImage || userData.image) ? (
                                            <img 
                                                ...
                                                src={previewImage || (userData.image?.startsWith("http") ? userData.image : `http://isetso-backend-lb-667158618.us-east-1.elb.amazonaws.com:8801/api/image/${userData.image}`)} 
                                                alt="profile" 
                                            />
                                        ) : (
                                            <img 
