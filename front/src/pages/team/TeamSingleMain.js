...
const TeamSingleMain = () => {
    ...
    const getDataAdmin = useCallback(async () => {
        ...
        try {
            const userId = await idUser();
            if (!userId) return;
            const response = await axios.get(`http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/auth/getAdminById/${userId}`);
            if (mountedRef.current) {
    ...
    const handleSaveClick = async (e) => {
        ...
        try {
            await axios.put(`http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/auth/updateprofil/${userData.id}`, formData, {
                headers: {
    ...
    const handleSaveClickAdmin = async (e) => {
        ...
        try {
            await axios.put(`http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/auth/updateAdminById/${adminData.id}`, updatedData);
            setIsEditing(false);
            await getDataAdmin();
        } catch (error) {
            console.error('Erreur:', error);
                                    ...
                                    <label style={imageLabelStyle}>
                                        {(previewImage || userData.image) ? (
                                            <img 
                                                ...
                                                src={previewImage || (typeof userData.image === 'string' && userData.image.startsWith("http") ? userData.image : typeof userData.image === 'string' ? `http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/image/${userData.image}` : teamImg1)} 
                                                alt="profile" 
                                            />
                                        ) : (
                                            <img 
