...
const CurriculumPart = () => {
    ...
    const fetchUserInfo = async () => {
        try {
            ...
            const response = await axios.get(`http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/users/${userId}`);
    ...
    const fetchCourse = async () => {
        try {
            const response = await axios.get(`http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/cours/getCourse/${id}`);
    ...
    const fetchTravauxByActivite = async (activiteId) => {
        try {
            const userId = await idUser();
            const response = await axios.get(`http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/travaux/getByActiviteAndEtudiant/${activiteId}/${userId}`);
    ...
    const getChapitreAndActivite = async () => {
        try {
            const response = await axios.get(`http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/chapitre/getChapitreAndActivite/${id}`);

            const chapitresAvecRessources = await Promise.all(
                response.data.map(async (chap, chapIndex) => {
                    try {
                        const resResponse = await axios.get(`http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/ressource/getAllRessourceId/${chap.id_chapitre}`);
                        ...
                        if (userId && userId !== 0) {
                            try {
                                const avcResponse = await axios.get(`http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/avc/avc/${id}/${userId}`);
    ...
    const fetchQuiz = async () => {
        try {
            const response = await axios.get(`http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/quiz/getQuiz/${id}`);
    ...
    const submitavc = async (nChapN) => {
        ...
        try {
            await axios.post("http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/avc/createOrUpdateAvc", {
                idCours: id,
                iduser: userid,
                chapN: nch
            });
    ...
    const handleSubmitTravail = async (e) => {
        ...
        try {
            ...
            await axios.post("http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/travaux/create", formData, {
    ...
    const handleSubmitReponses = async (activiteId) => {
        ...
        try {
            const userId = await idUser();
            await axios.post("http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/reponses/create", {
    ...
    const renderActivityContent = (activite) => {
            ...
            try {
                ...
                return (
                    <div>
                        ...
                        {devoirData.fichier && (
                            <a
                                href={devoirData.fichier && devoirData.fichier.includes("http") ? devoirData.fichier : `http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/activite/fichier/${devoirData.fichier}`}
                                target="_blank"
        ...
        if (activite.categorie === "video_interactive") {
            try {
                ...
                const videoSrc = videoData.video && videoData.video.includes("http") ? videoData.video : `http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/activite/video/${videoData.video}`;
                return (
                    <div>
                ...
                const videoSrc = activite.contenu && activite.contenu.includes("http") ? activite.contenu : `http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/activite/video/${activite.contenu}`;
                return (
                    <div>
                        <video controls style={styles.modalVideo}>
                            <source src={videoSrc} type="video/mp4" />
    ...
    const renderRessourceContent = (ressource) => {
        const nomFichier = ressource.fichier || '';
        const estVideo = nomFichier.endsWith('.mp4') || nomFichier.endsWith('.mov') || nomFichier.endsWith('.avi') || nomFichier.includes('video');

        if (ressource.type_ressource === 'video' || estVideo) {
            const videoSrc = nomFichier.includes("http") ? nomFichier : `http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/ressource/video/${nomFichier}`;
            return (
    ...
    const styles = {
        ...
        chapterHeaderCompleted: { background: 'linear-gradient(135deg, #28a745 0%, #1e7e34 100%)' },
        chapterNumber: { width: '35px', height: '35px', background: 'rgba(255,255,255,0.2)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold' },
        chapterTitle: { flex: 1, fontSize: '16px', fontWeight: '600' },
        chapterStats: { fontSize: '12px', opacity: 0.8 },
        checkIcon: { marginLeft: '10px', fontSize: '16px' },
        section: { marginBottom: '20px' },
        sectionTitle: { padding: '10px 15px', background: '#f8f9fa', borderRadius: '8px', fontSize: '14px', fontWeight: '600', color: '#ff5421', marginBottom: '10px', borderLeft: '3px solid #ff5421' },
        activityItem: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 15px', marginBottom: '8px', background: '#fff', border: '1px solid #eee', borderRadius: '8px', cursor: 'pointer', transition: 'all 0.2s ease' },
        activityLeft: { display: 'flex', alignItems: 'center', gap: '10px' },
        activityIcon: { width: '32px', height: '32px', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px' },
        activityTitle: { fontSize: '14px', color: '#333' },
        activityDuration: { fontSize: '12px', color: '#999', background: '#f5f5f5', padding: '4px 10px', borderRadius: '20px' },
        ressourceItem: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 15px', marginBottom: '8px', background: '#f0f7ff', border: '1px solid #e0e8f0', borderRadius: '8px', cursor: 'pointer', transition: 'all 0.2s ease' },
        ressourceIcon: { width: '28px', height: '28px', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px' },
        ressourceTitle: { fontSize: '13px', color: '#2c3e50', fontWeight: '500' },
        downloadBtn: { background: '#ff5421', color: 'white', padding: '5px 12px', borderRadius: '5px', fontSize: '11px', textDecoration: 'none', transition: 'all 0.2s ease' },
        watchBtn: { background: '#17a2b8', color: 'white', padding: '5px 12px', borderRadius: '5px', fontSize: '11px', textDecoration: 'none', transition: 'all 0.2s ease' },
        contentPanel: { padding: '15px', background: '#fafafa', borderRadius: '8px', marginTop: '8px', marginBottom: '15px', marginLeft: '20px', borderLeft: '2px solid #ff5421' },
        textContent: { fontSize: '14px', lineHeight: '1.6', color: '#555' },
        modalImage: { maxWidth: '100%', maxHeight: '80vh', borderRadius: '8px' },
        modalVideo: { width: '100%', maxHeight: '80vh', borderRadius: '8px' },
        travauxSection: { marginTop: '15px', padding: '15px', background: '#f9f9f9', borderRadius: '8px', border: '1px solid #eee' },
        travauxHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px', flexWrap: 'wrap', gap: '10px' },
        travauxTitle: { fontSize: '14px', fontWeight: '600', color: '#ff5421', margin: 0 },
        submitButton: { background: '#28a745', color: '#fff', border: 'none', padding: '8px 16px', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', display: 'flex', alignItems: 'center', gap: '6px' },
        travailCard: { background: '#fff', border: '1px solid #e0e0e0', borderRadius: '8px', padding: '12px', marginBottom: '10px' },
        travailStatus: (status) => ({ display: 'inline-block', padding: '4px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: 500, background: status === 'note' ? '#d4edda' : '#fff3cd', color: status === 'note' ? '#155724' : '#856404' }),
        ...
        uploadArea: { border: '2px dashed #ddd', borderRadius: '12px', padding: '20px', textAlign: 'center', cursor: 'pointer', marginBottom: '15px', transition: 'all 0.2s' },
        fileInfo: { fontSize: '12px', color: '#666', marginTop: '5px' },
        noteDisplay: { background: '#d4edda', padding: '8px 12px', borderRadius: '8px', textAlign: 'center', marginTop: '10px' },
        noteValue: { fontSize: '18px', fontWeight: 'bold', color: '#155724' },
        videoContainer: { marginTop: '10px', borderRadius: '8px', overflow: 'hidden' }
    };
                    ...
                    <div key={chapitre.id_chapitre} style={{ marginBottom: '15px' }}>
                        ...
                        >
                            ...
                            <div style={styles.chapterStats}>
                                ...
                                {isChapterCompleted && <span style={styles.checkIcon}> ✅</span>}
                            </div>
                        </div>
                                ...
                                {chapitre.ressources && chapitre.ressources.length > 0 && (
                                    <div style={styles.section}>
                                        ...
                                        {chapitre.ressources.map((ressource, resIdx) => {
                                            ...
                                            const nomFichier = ressource.fichier || '';
                                            const estVideo = ressource.type_ressource === 'video' || nomFichier.endsWith('.mp4') || nomFichier.endsWith('.mov') || nomFichier.includes('video');

                                            return (
                                                <div key={ressource.id}>
                                                    <div style={styles.ressourceItem} onClick={() => toggleRessource(ressource.id)}>
                                                        ...
                                                        {estVideo ? (
                                                            ...
                                                            <a href={nomFichier.startsWith('http') ? nomFichier : `http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/ressource/fichier/${nomFichier}`} target="_blank" rel="noopener noreferrer" style={styles.downloadBtn} onClick={(e) => e.stopPropagation()}>
                                                        ...
                                                        <div style={{...styles.contentPanel, marginBottom: '8px'}}>
                                                            ...
                                                            {estVideo && (
                                                                <div style={styles.videoContainer}>
                                                                    <video controls style={{ width: '100%', maxHeight: '400px', borderRadius: '8px', marginTop: '10px' }} controlsList="nodownload">
                                                                        <source src={nomFichier.includes("http") ? nomFichier : `http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/ressource/video/${nomFichier}`} type="video/mp4" />
                                                                        Votre navigateur ne supporte pas la lecture de vidéos.
                                                                    </video>
                                                                </div>
                                                            )}
                                ...
                                {chapitre.activites && chapitre.activites.length > 0 && (
                                    <div style={styles.section}>
                                        ...
                                        {chapitre.activites.map((activite, actIdx) => {
                                            ...
                                            return (
                                                <div key={activite.id}>
                                                    <div style={styles.activityItem} onClick={() => toggleActivite(idx, activite.id)}>
                                                        <div style={styles.activityLeft}>
                                                                        ...
                                                                        <div style={styles.travailCard}>
                                                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                                                                                <div style={{ flex: 1 }}>
                                                                                    ...
                                                                                    {travailExistant.lien && <a href={travailExistant.lien} target="_blank" rel="noopener noreferrer" style={{ fontSize: '13px', color: '#ff5421', display: 'inline-block', marginTop: '8px' }}><i className="fa fa-link"></i> Voir le lien</a>}
                                                                                    {travailExistant.fichier && <a href={`http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/travaux/fichier/${travailExistant.fichier}`} target="_blank" rel="noopener noreferrer" style={{ fontSize: '13px', color: '#ff5421', display: 'inline-block', marginLeft: '15px', marginTop: '8px' }}><i className="fa fa-download"></i> Télécharger</a>}
                                                                                </div>
                                                                            </div>
                                                                            {travailExistant.note && <div style={styles.noteDisplay}><div style={styles.noteValue}>Note: {travailExistant.note}/20</div>{travailExistant.commentaire && <div style={{ fontSize: '12px', marginTop: '5px' }}><strong>Commentaire:</strong> {travailExistant.commentaire}</div>}</div>}
                                                                        </div>
                                                                    ) : (
                    ...
                    <div style={styles.modalContent} onClick={e => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleSubmitTravail}>
                            <div style={styles.modalBody}>
                                <input type="text" name="titre" placeholder="Titre de votre travail" value={nouveauTravail.titre} onChange={handleTravailChange} style={styles.input} required />
                                <textarea name="description" placeholder="Description / Réponse" value={nouveauTravail.description} onChange={handleTravailChange} style={styles.textarea} required />
                                <input type="url" name="lien" placeholder="Lien vers votre travail (Google Drive, etc.)" value={nouveauTravail.lien} onChange={handleTravailChange} style={styles.input} />
