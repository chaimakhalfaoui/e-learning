...
const FaqPart = () => {
    ...
    const fetchQuiz = async () => {
        ...
        try {
            ...
            try {
                const response = await axios.get(`process.env.REACT_APP_API_URL/repense/fetchFirstFalseResponseQuestion/${idq}/${userid}`);
                if (response.data !== 0) {
                    setQuestion(response.data);
                    console.log(response.data)
                } else {
                    try {
                        const response = await axios.get(`process.env.REACT_APP_API_URL/repense/getQuizScore/${idq}/${userid}`);
    ...
    const deleteResponses = async () => {
        const userid = await idUser();
        try {
            await axios.delete(`process.env.REACT_APP_API_URL/repense/deleteResponsesByQuizAndUser/${idQuiz}/${userid}`);
            setScore(null);
    ...
    const markAllResponsesAsFalse = async () => {
        try {
            ...
            const quizResponse = await axios.get(`process.env.REACT_APP_API_URL/quiz/getQuiz/${id}`);
            const idq = quizResponse.data[0].id;
            console.log("Quiz ID:", idq);
            
            // Récupérer la première question sans réponse de l'utilisateur
            const firstFalseResponse = await axios.post(`process.env.REACT_APP_API_URL/repense/create-false/${idq}/${userid}`);
    ...
    const saveResponse = async (e) => {
        ...
        try {
            await axios.post('process.env.REACT_APP_API_URL/repense/createRepense', {
    ...
    const createCertificate = async () => {
        ...
        try {
            await axios.post('process.env.REACT_APP_API_URL/certaficat/createCertificate', {
                idCours: id, 
                idUser: userid,
                note: formattedScore 
            });
    ...
    const exportCertificate = async () => {
        const userid = await idUser();
        try {
            const response = await axios.get(`process.env.REACT_APP_API_URL/certaficat/getCertificateByIds/${id}/${userid}`);
