...
const FaqPart = () => {
    ...
    const fetchQuiz = async () => {
        ...
        try {
            ...
            try {
                const response = await axios.get(`http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/repense/fetchFirstFalseResponseQuestion/${idq}/${userid}`);
                if (response.data !== 0) {
                    setQuestion(response.data);
                    console.log(response.data)
                } else {
                    try {
                        const response = await axios.get(`http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/repense/getQuizScore/${idq}/${userid}`);
    ...
    const deleteResponses = async () => {
        const userid = await idUser();
        try {
            await axios.delete(`http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/repense/deleteResponsesByQuizAndUser/${idQuiz}/${userid}`);
            setScore(null);
    ...
    const markAllResponsesAsFalse = async () => {
        try {
            ...
            const quizResponse = await axios.get(`http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/quiz/getQuiz/${id}`);
            const idq = quizResponse.data[0].id;
            console.log("Quiz ID:", idq);
            
            // Récupérer la première question sans réponse de l'utilisateur
            const firstFalseResponse = await axios.post(`http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/repense/create-false/${idq}/${userid}`);
    ...
    const saveResponse = async (e) => {
        ...
        try {
            await axios.post('http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/repense/createRepense', {
    ...
    const createCertificate = async () => {
        ...
        try {
            await axios.post('http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/certaficat/createCertificate', {
                idCours: id, 
                idUser: userid,
                note: formattedScore 
            });
    ...
    const exportCertificate = async () => {
        const userid = await idUser();
        try {
            const response = await axios.get(`http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/certaficat/getCertificateByIds/${id}/${userid}`);
