...
const InstructorPart = () => {
    ...
    const fetchInstructor = async () => {
        ...
        try {
            ...
            const nameResponse = await axios.get(`http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/cours/getUserNameByCourseId/${id}`);
            setName(nameResponse.data);
            
            // Récupérer l'email de l'enseignant
            const idResponse = await axios.get(`http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/cours/getUserIdByCourseId/${id}`);
            const enseignantId = idResponse.data.id_user;
            
            if (enseignantId) {
                const userResponse = await axios.get(`http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/users/${enseignantId}`);
