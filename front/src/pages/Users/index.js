...
const Users = () => {
    ...
    const fetchUsers = async () => {
        ...
        setError(null);
        try {
            const response = await axios.get("http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/users");
    ...
    const handleDelete = async (id, username) => {
        if (window.confirm(`Voulez-vous vraiment supprimer l'utilisateur "${username}" ?`)) {
            try {
                await axios.delete(`http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/users/${id}`);
