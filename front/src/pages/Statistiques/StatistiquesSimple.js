        ...
        (async () => {
            try {
                const [sRes, mRes, cRes] = await Promise.all([
                    axios.get('http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/auth/getStatistics')
                        .catch(() => ({ data: null })),
                    axios.get('http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/auth/monthlyInscriptions')
                        .catch(() => ({ data: [] })),
                    axios.get('http://isetso-alb-1947778921.us-east-1.elb.amazonaws.com/api/auth/coursParCategorie')
                        .catch(() => ({ data: [] }))
                ]);
