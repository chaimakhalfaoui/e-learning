...
const FaqPart = () => {
    ...
    const fetchQuiz = async () => {
        ...
        try {
            ...
            try {
                const response = await axios.get(`http://isetso-backend-lb-667158618.us-east-1.elb.amazonaws.com:8801/api/repense/fetchFirstFalseResponseQuestion/${idq}/${userid}`);
                if (response.data !== 0) {
                    setQuestion(response.data);
                    console.log(response.data)
                } else {
                    try {
                        const response = await axios.get(`http://isetso-backend-lb-667158618.us-east-1.elb.amazonaws.com:8801/api/repense/getQuizScore/${idq}/${userid}`);
    ...
    const deleteResponses = async () => {
        const userid = await idUser();
        try {
            await axios.delete(`http://isetso-backend-lb-667158618.us-east-1.elb.amazonaws.com:8801/api/repense/deleteResponsesByQuizAndUser/${idQuiz}/${userid}`);
            setScore(null);
    ...
    const markAllResponsesAsFalse = async () => {
        try {
            ...
            const quizResponse = await axios.get(`http://isetso-backend-lb-667158618.us-east-1.elb.amazonaws.com:8801/api/quiz/getQuiz/${id}`);
            const idq = quizResponse.data[0].id;
            console.log("Quiz ID:", idq);
            
            // Récupérer la première question sans réponse de l'utilisateur
            const firstFalseResponse = await axios.post(`http://isetso-backend-lb-667158618.us-east-1.elb.amazonaws.com:8801/api/repense/create-false/${idq}/${userid}`);
    ...
    const saveResponse = async (e) => {
        ...
        try {
            await axios.post('http://isetso-backend-lb-667158618.us-east-1.elb.amazonaws.com:8801/api/repense/createRepense', {
    ...
    const createCertificate = async () => {
        ...
        try {
            await axios.post('http://isetso-backend-lb-667158618.us-east-1.elb.amazonaws.com:8801/api/certaficat/createCertificate', {
                idCours: id, 
                idUser: userid,
                note: formattedScore 
            });
    ...
    const exportCertificate = async () => {
        const userid = await idUser();
        try {
            const response = await axios.get(`http://isetso-backend-lb-667158618.us-east-1.elb.amazonaws.com:8801/api/certaficat/getCertificateByIds/${id}/${userid}`);
